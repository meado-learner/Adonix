import { smsg } from "./core/simple.js"
...
import ws from "ws"
import { defaultSettings } from "./config/main.js"
...
export async function handler(chatUpdate) {
    ...
    const subBotPath = path.join(global.jadi, botNumber);
    const configPath = path.join(subBotPath, 'config.json');
    ...
    if (fs.existsSync(configPath)) {
        try {
            ...
            console.error("Error reading sub-bot config, using defaults:", e);
        }
    }
...
unwatchFile(file)
console.log(chalk.magenta("Se actualizo 'events.js'"))
})
